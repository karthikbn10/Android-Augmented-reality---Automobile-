...
Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargets_loadTrackerData(JNIEnv *, jobject)
{
    ...
    // Load the data sets:
    if (!dataSetStonesAndChips->load("CarSpecs.xml", QCAR::DataSet::STORAGE_APPRESOURCE))
    {
    ...
    for(int tIdx = 0; tIdx < state.getNumActiveTrackables(); tIdx++)
    {
        ...
        QCAR::Matrix44F modelViewMatrix =
         ...
         if (trackable->getId() != lastTrackableId) {
            ...
            jmethodID method = env->GetMethodID(javaClass, "onImageTracked", "(Ljava/lang/String;)V");
